...
async def fetch_breeds():  # gets every breed in thecatapi
    ...
    if breed_cache:  # use cached list
        return breed_cache
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get("https://api.thecatapi.com/v1/breeds", headers=headers) as resp:
            if resp.status == 200:
                ...
                raise Exception(f"Failed to fetch breeds: {error_message}")

async def fetch_breed_info(breed_id: str):
    headers = {"x-api-key": CATAPIKEY}
    async with aiohttp.ClientSession() as session:
...
async def fetch_cat_image(breed_id: str = None):
    headers = {"x-api-key": CATAPIKEY}
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get("https://api.thecatapi.com/v1/images/search", headers=headers, params=params) as resp:
            if resp.status == 200:
                ...
                breed_info = data[0].get("breeds", [])  # fetches breed info
